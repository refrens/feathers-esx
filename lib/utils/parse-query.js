  ...
  const bool = Object.keys(query).reduce((result, key) => {
    ...
    if (type !== 'object') {
      ...
      if (type === 'array') {
        ...
        filteredValues.forEach((v) => result.filter.push({ term: { [key]: v } }));
      } else {
    ...
    Object.keys(value)
      ...
      .forEach((criterion) => {
        const criterionValue = value[criterion];

        // Skip null values in criteria to avoid Elasticsearch errors
        ...
        result[section].push({
          [term]: {
            [key]: operand ? { [operand]: processedValue } : processedValue,
          },
        });
      });
